#ifndef __Conjunto2_h
#define __Conjunto2_h
...
Conjunto<T> Conjunto<T>::operator+(const Conjunto<T> &other) {
    ...
    if(&other==this)    //Caso os conjuntos sejam iguais, o resultado sera o proprio conjunto: A + A = A
        return *this;
    
    Conjunto<T> resultado(this->tamarray() + other.tamarray());

    for(int i=0; i<this->num_elementos; i++)
        resultado.insere(this->elementos[i]);
    
    for(int i=0; i<other.num_elementos; i++)
        resultado.insere(other.elementos[i]);
...
Conjunto<T> Conjunto<T>::operator*(const Conjunto<T> &other) {
    ...
    int tamanhoIntersecao = this->tamarray();
    if(other.tamarray() < tamanhoIntersecao) tamanhoIntersecao = other.tamarray();

    Conjunto<T> resultado(tamanhoIntersecao);
...
Conjunto<T> Conjunto<T>::operator-(const Conjunto<T> &other) {
    ...
    Conjunto<T> resultado(this->tamarray());
