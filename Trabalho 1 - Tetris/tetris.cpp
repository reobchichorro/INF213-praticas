...
void Tetris::alocarColunaAteAltura(const int c, const int l) {
    if(l < getAltura(c))    //Caso a altura nova seja menor do que a altura antiga, nao precisamos alocar nada.         
        return;

    char* temp = new char[l];   //Variavel temporaria que armazena os novos valores da coluna.
    for(int i=0; i<getAltura(c); i++)   //Ate a altura antiga, usamos os valores antigos da coluna.
        temp[i] = jogo[c][i];
    for(int i=getAltura(c); i<l; i++)   //Dai em diante, preenchemos com espacos em branco.
        temp[i] = ' ';
