...
public:
	...
	int size() const {return calcSize(begin());};
	int calcSize(const iterator curr) const;
...
template<class T>
int MyList2<T>::calcSize(const iterator curr) const {
	if(curr == end())
		return 0;
	return 1 + calcSize(next(curr));
}
...
void MyList2<T>::reverse(Node<T>* curr) {
	...
	if(dataFirst->next != NULL) {
		...
		dataFirst->next = curr;
		if(curr != NULL) curr->prev = dataFirst;
		curr = dataFirst;
		curr->prev = NULL;
		dataFirst = temp;
		dataFirst->prev = NULL;
